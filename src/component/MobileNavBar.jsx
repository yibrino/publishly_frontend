...
export const MobileNavBar = () => {
  ...
  return (
    <nav className="block sm:hidden">
      <div
        className="fixed bottom-20 right-5 w-100 h-100 p-3 text-2xl rounded-full block xl:hidden cursor-pointer
                bg-blue-600 hover:bg-blue-800 z-10"
      >
        <GrEdit onClick={() => dispatch(openPostModal())}></GrEdit>
      </div>
      ...
      <ul className="fixed bottom-0 w-full sm:pl-2 flex justify-around bg-slate-300 z-10">
        <li>
          <NavLink
            to="/home"
            className="flex py-4 gap-3 px-3 cursor-pointer hover:bg-slate-200 rounded-[15rem] active:bg-slate-300"
          >
            {({ isActive }) =>
              isActive ? (
                <>
